...
| Crawler limits | URL, byte, redirect and request concurrency bounds; per-project/global scan caps; frontier lease batch/duration, persisted attempt cap and retry base delay; connect/read duration bounds; dedicated robots DNS/open/read deadlines, 500 KiB-or-higher response cap and at-most-five redirects; sitemap well-known switch plus DNS/open/read, compressed/decompressed byte, document/entry, XML/index depth and redirect limits | none |

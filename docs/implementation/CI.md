...
| `Security and provider contracts` | Fresh advisory data, static analysis, authentication and tenant-isolation regressions, centralized destination/transport SSRF corpus, hostile network fixtures and adapter contracts | `bin/bundler-audit check --update`, `bin/importmap audit`, `bin/brakeman --quiet --config-file config/brakeman.yml`, `bin/authentication-security`, `bin/tenancy-security`, then the destination-policy, malicious-network, safe-client, pinned-transport and adapter-contract tests listed in `.github/workflows/ci.yml` |

  ...
  class DiagnosticAuthorizer
    def call(organization_id:, authorization:)
      allowed = authorization&.respond_to?(:allow?) && authorization.allow? &&
        authorization.respond_to?(:permission_key) && authorization.permission_key == "plans.read" &&
      ...
      raise AccessDenied.new(reason_code: "entitlement_diagnostics_forbidden") unless allowed

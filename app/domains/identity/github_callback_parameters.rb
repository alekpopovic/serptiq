...
module Identity
  class GithubCallbackParameters
    ...
    def authorization_code!
      ...
      raise malformed("github_callback_parameters_invalid") unless valid
    ...
    def raise_provider_error!
      return unless error
      raise malformed("github_authorization_error_invalid") unless ERROR_PATTERN.match?(error) && code.nil?
      ...
      when "access_denied"
        raise ProviderError.new(
          ...
          reason_code: "github_authorization_denied"
        )
      when "server_error", "temporarily_unavailable"
        raise ProviderError.new(
          category: "unavailable",
          operation: "authorization_response",
          reason_code: "github_authorization_unavailable"
        )
      else
        raise malformed("github_authorization_error_unknown")
      end
    end

    def inspect
      "#<#{self.class.name} state=[FILTERED] code=#{code ? '[FILTERED]' : 'nil'} error=#{error.inspect}>"
    end
